...
export default function ChooseLevel({birthDay}) {
    ...
    const getLevels = (birthYear) => {
        if (birthYear === 2021) {
            ...
            return [
                ...
                { text: "Orange", level: '4' },
                { text: "Vert", level: '5' },
                { text: 'Vous avez un doute sur votre niveau et vous n\'avez pas de classement.', level: "" }
            ];
        } else if (birthYear <= 2014 && birthYear >= 2007) {
            return [
                { text: 'Débutant', level: '6' },
                { text: "Jaune 1 (jeu sur terrain orange avec balles adaptées)", level: '6' },
                { text: "Jaune 2 (jeu sur grand terrain avec balles vertes)", level: '7' },
                { text: "Jaune 3 (jeu sur terrain normal avec balles dures)", level: '8' },
                { text: "Jaune 4 (jeu sur terrain normal avec balles dures, classé entre 30/3 et 30/1 ou niveau)", level: '9' },
                { text: 'Vous avez un doute sur votre niveau et vous n\'avez pas de classement.', level: "" }
            ];
    ...
    <div className='choose-level-container' >
        ...
        {levelToPropose?.map((level, index) => {
            return(
                <div className='levels' onClick={() => setLevelChoose(level.level)} key={index}>
                    <div><h3>{level.text}</h3></div>
                </div>
            )
        })}
      <button onClick={handleSubmit} type="submit" className="submit-btn">Valider</button>
